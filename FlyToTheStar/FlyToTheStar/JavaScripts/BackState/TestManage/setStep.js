...
SetStep.prototype.init=function(arg){
    ...
    stepsHtml.find('li').css('width','75px').css('marginRight',w_li*2-80)
    ...
    if(this.opt.showBtn){
        ...
        this.prevBtn.on('click',function(){
            var chebox = $("input[name=radioexamgroup]:checked");   // 获取试卷
            // 【第二步】：判断是否勾选试卷   
            // _that.opt.curStep是步骤的索引(是否是在第2步)
            if(chebox.length == 0 && _that.opt.curStep == 2){
                alert("至少选择一张试卷进行考试");
                return;
            }
            ...
            if(_that.opt.curStep == 3){
                if($("#txtexamtime").val() == "" || $("#txtpass").val() == "")
                {
                    alert("请输入考试时长和及格分数");
                    return;
                }
    ...
    if(this.opt.clickAble){
        stepsHtml.find('li').on('click',function(){
            ...
            // 判断第一页中的下一页按钮能不能用，不能用代表考试名称是重复的
            if (_that.opt.curStep == 1 && $("#nextBtn").attr("disabled") == "disabled") {
